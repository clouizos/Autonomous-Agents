...
\label{PEval}
In Markov Decision Processes, one of the important steps is the evaluation step, which is to find the state-value functions $V^\pi$ for an arbitrary policy, $\pi$. The state-value functions estimate the value of a state in terms of the expected return that can be obtained from that state. The equation for calculating $V^\pi$ can be seen at ~\ref{oriV}:
\begin{align}
